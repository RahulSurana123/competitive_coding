...
int main()
{
    ...
    while(t--) {
        ...
        int x;
        cin >> x;
        FOR(i,x) { ll a; cin >> a; cord[0].pb(a); }
        cin >> x;
        FOR(i,x) { ll a; cin >> a; cord[1].pb(a); }
        cin >> x;
        FOR(i,x) { ll a; cin >> a; cord[2].pb(a); }
        cin >> x;
        FOR(i,x) { ll a; cin >> a; cord[3].pb(a); }

        ll ans = 0;
        ans = max(ans,h*(cord[0][cord[0].size()-1] - cord[0][0]));
        ans = max(ans,h*(cord[1][cord[1].size()-1] - cord[1][0]));
        ans = max(ans,w*(cord[2][cord[2].size()-1] - cord[2][0]));
        ans = max(ans,w*(cord[3][cord[3].size()-1] - cord[3][0]));
